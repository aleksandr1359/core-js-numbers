...
function getParallelepipedDiagonal(a, b, c) {
  const sD = Math.sqrt(a ** 2 + b ** 2 + c ** 2);
  return sD;
}
...
function isPrime(n) {
  ...
  for (let index = 1; index <= n; index += 1) {
    if (n % index === 0) resList.push(index);
  }

  return resList.length === 2;
}
...
function getCountOfOddNumbers(number) {
  ...
  const x = numbAbs % 2 === 0 ? numbAbs / 2 : (numbAbs - 1) / 2 + 1;
  return x;
}
